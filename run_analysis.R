...
FecTrain <- read.table("UCI HAR Dataset/train/X_train.txt", header = FALSE)
...
subject <- rbind(SubTrain, subjectTest)
activity <- rbind(ActTrain, activityTest)
features <- rbind(FecTrain, featuresTest)
...
ColsWithMSTD <- grep(".*Mean.*|.*Std.*", names(completeData), ignore.case=TRUE)
requiredColumns <- c(ColsWithMSTD, 562, 563)
dim(completeData)
